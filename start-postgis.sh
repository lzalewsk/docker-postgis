...
# POSTGIS CLASTERING configuration if CLASTER_NODE ["MASTER"|"SLAVE"] is provided
# Usefe is: docker run [...] -e CLASTER_NODE='MASTER'
RESULT=`cat $CONF |grep 'PGIS_CLASTER' |wc -l`
if [[ ${RESULT} == '1' ]]
then
	echo 'PGIS_CLASTER already done'
else
	echo 'Preparing PGIS_CLASTER conf'
	if [ -z "$CLASTER_NODE" ]; then
		CLASTER_NODE=None
	fi
	if [ "$CLASTER_NODE" == "MASTER" ]; then
		echo "#PGIS_CLASTER MASTER PARAM" >> $CONF
		echo "wal_level = hot_standby" >> $CONF
		echo "max_replication_slots = 3" >> $CONF
		echo "max_wal_senders = 3" >> $CONF
	fi
	if [ "$CLASTER_NODE" == "SLAVE" ]; then
		echo "#PGIS_CLASTER SLAVE PARAM" >> $CONF
		echo "wal_level = hot_standby" >> $CONF
